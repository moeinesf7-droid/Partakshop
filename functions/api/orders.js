...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const result = await env.DB
      .prepare(`SELECT id,name,price,active FROM products WHERE active=1 AND id IN (${placeholders})`)
      .bind(...ids).all();
    ...
    for (const raw of items) {
      ...
      if (!p) return json({error:'یکی از محصولات دیگر موجود نیست'},400);
    ...
    const code = 'PT-' + Date.now().toString().slice(-7) +
      Math.floor(10+Math.random()*90);

    const created = new Date().toISOString();

    const status = payment==='online_pending'
      ? 'در انتظار پرداخت آنلاین'
      : 'در انتظار بررسی';

    const statements = [
      env.DB.prepare(
        `INSERT INTO orders(code,name,phone,address,payment,total,status,created_at)
         VALUES(?,?,?,?,?,?,?,?)`
    ...
    for (const item of normalized) {
      statements.push(
        env.DB.prepare(
          `INSERT INTO order_items(order_code,product_id,product_name,price,qty)
           VALUES(?,?,?,?,?)`
    ...
    return json({error:'خطا در ثبت سفارش'},500);
  }
}
...
export async function onRequestGet({ request, env }) {
  ...
  const order = await env.DB
    .prepare(`SELECT code,total,status,created_at FROM orders WHERE code=?`)
    .bind(code).first();

  if (!order)
    return json({error:'سفارشی با این کد پیدا نشد'},404);
